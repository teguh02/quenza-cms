...
final class OptionSeeder extends Seeder
{
    ...
    {
        $options = [
            'site_title' => [(string) $this->app->config('app.name', 'Quenza CMS'), 1],
            ...
            'active_locale' => [(string) $this->app->config('app.locale', 'id'), 1],
            'site_timezone' => [(string) $this->app->config('app.timezone', 'Asia/Jakarta'), 1],
            'active_theme' => [(string) $this->app->config('app.active_theme', 'quenza_default'), 1],
        ];
